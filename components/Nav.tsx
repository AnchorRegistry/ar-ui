...
export default function Nav() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-[#2E4270] bg-[#152038]">
      <div className="mx-auto flex h-[52px] max-w-[960px] items-center justify-between px-8">
        ...
        <Link href="/" className="flex items-center gap-2 text-[15px] font-medium">
          ...
          <span>
            ...
            <span className="text-muted-slate align-super text-[10px]">™</span>
          </span>
        </Link>
        ...
        <ul className="hidden items-center gap-7 md:flex">
          <li>
            <Link href="/#how-it-works" className="text-[13px] text-muted-slate transition-colors hover:text-off-white">
              How it works
            </Link>
          </li>
          <li>
      ...
      {menuOpen && (
        <div className="border-t border-[#2E4270] bg-[#152038] px-8 py-4 md:hidden">
          <ul className="flex flex-col gap-4">
            {['/#how-it-works', '/verify', '/docs', '/#pricing'].map((href, i) => (
              <li key={i}>
                <Link href={href} className="text-[14px] text-muted-slate" onClick={() => setMenuOpen(false)}>
                  {['How it works', 'Verify', 'Docs', 'Pricing'][i]}
                </Link>
              </li>
            ))}
            <li>
